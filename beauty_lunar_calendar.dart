 ...
 */
 class LunarCalendar {
	 ...
	 int month;
	 int day;
	 String lunarMonth; // 农历的月份
	 bool leap;
	 int leapMonthInt = 0; // 闰的是哪个月

   var chineseNumber = ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二"];
	...
	// 农历部分假日
	var lunarHoliday = ["0101 春节", "0115 元宵", "0505 端午", "0707 情人", "0715 中元", "0815 中秋", "0909 重阳", "1208 腊八", "1224 小年", "0100 除夕"];
	//var lunarHoliday =["0101 Spring Festival", "0115 The Lantern Festival", "0505 The Dragon Boat Festival", "0707 The Double Seventh Day", "0715 The ghost festival", "0815 The Mid-Autumn Festival ", "0909 The Double Ninth Day ", "1208 Laba Festival",  "0100 New Year's Eve"];
	...
	// var solarHoliday =[ //
	// 		"0101 New year's Day", "0214 Valentine's Day", "0308 Women's Day", "0312 Tree planting Festival", "0401 April Fools'Day", "0501 Labor Day", //
	// 		"1001 National Day"];
...
int leapDays(int y) {
	...
	int leapMonth(int y) {
		int result = lunarInfo[y - 1900] & 0xf;
		return result;
	}
 ...
 String cyclicalm(int num) {
	...
	// ====== 传入 offset 传回干支, 0=甲子
	String cyclical(int year) {
		int num = year - 1900 + 36;
		return (cyclicalm(num));
	}

	String getChinaDayString(int day) {
	 ...
	 * @return
	 */
	 String getLunarDate(int year_log, int month_log, int day_log, bool isday) {
		int yearCyl, monCyl, dayCyl;
		// int leapMonth = 0;
		String nowadays;
		DateTime baseDate = null;
		DateTime nowaday = null;

		baseDate = DateTime.parse("1900-01-31");
		nowaday = DateTime(year_log,month_log,day_log);

		// 求出和1900年1月31日相差的天数
		int offset =(nowaday.millisecondsSinceEpoch - baseDate.millisecondsSinceEpoch) ~/ 86400000;
		dayCyl = offset + 40;
		monCyl = 14;
		...
		yearCyl = iYear - 1864;
		leapMonthInt = leapMonth(iYear); // 闰哪个月,1-12
		leap = false;
		...
		for (iMonth = 1; iMonth < 13 && offset > 0; iMonth++) {
			// 闰月
			if (leapMonthInt > 0 && iMonth == (leapMonthInt + 1) && !leap) {
				--iMonth;
				leap = true;
			...
			offset -= daysOfMonth;
			// 解除闰月
			if (leap && iMonth == (leapMonthInt + 1))
				leap = false;
			if (!leap)
				monCyl++;
		}
		// offset为0时，并且刚才计算的月份是闰月，要校正
		if (offset == 0 && leapMonthInt > 0 && iMonth == leapMonthInt + 1) {
			if (leap) {
				leap = false;
			} else {
				leap = true;
		...
		if (!isday) {
			...
			for (int i = 0; i < solarHoliday.length; i++) {
				...
				String sdv = solarHoliday[i].replaceAll(sd,"").trim();//solarHoliday[i].split(" ")[1]; // 节假日的名称
				String smonth_v = month_log.toString();
				String sday_v = day_log.toString();
				String smd = "";
				if (month_log < 10) {
					smonth_v = "0" + month_log.toString();
				}
				if (day_log < 10) {
					sday_v = "0" + day_log.toString();
				}
				smd = smonth_v + sday_v;
				if (sd.trim()==(smd.trim())) {
					return sdv;
			...
			for (int i = 0; i < lunarHoliday.length; i++) {
				...
				String ldv = lunarHoliday[i].replaceAll(ld,"").trim();//.split(" ")[1]; // 节假日的名称
				String lmonth_v = month.toString();
				String lday_v = day.toString();
				String lmd = "";
				if (month < 10) {
					lmonth_v = "0" + month.toString();
				}
				if (day < 10) {
					lday_v = "0" + day.toString();
				}
				lmd = lmonth_v + lday_v;
				if (ld.trim()==(lmd.trim())) {
					return ldv;
  ...
  void setLunarMonth(String lunarMonth) {
